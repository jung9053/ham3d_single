...
!#
!#     This is exteded version of 2D for 3D simulation by Yong Su Jung
...
!#     flux(5)     - Vector of flux across face is multiplied by face area.
...
!
!------------------------------------------------------------------------------
subroutine flux_roe3d(nxyz,ql,qr,flx,specRadius,gam)
!------------------------------------------------------------------------------
implicit none
!------------------------------------------------------------------------------
real*8  :: nxyz(3)
...
!------------------------------------------------------------------------------
       ...
       vl  = ql(3)/ql(1) !v
       wl  = ql(4)/ql(1) !w
       pl  = gm1*( ql(5) - 0.5 * rol * (ul*ul + vl*vl + wl*wl) ) !p
       hl  = (ql(5) + pl)/rol !h
       cl  = sqrt(gam*pl/rol) !a
       ...
       flx(1:5)=flux(1:5)*area
       specradius=specradius*area
!------------------------------------------------------------------------------
end subroutine flux_roe3d
